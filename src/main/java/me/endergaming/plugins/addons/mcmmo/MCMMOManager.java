package me.endergaming.plugins.addons.mcmmo;
...
public class MCMMOManager extends Addon {
    private static final int MAX_LEVEL = 1000;

    public MCMMOManager(@NotNull final ServerAdditionsPlus instance, @NotNull String reqPlugin, @NotNull String... reqManagers) {
    ...
    public String getFormattedWeightedLevel(Player player) {
        int level = this.calculateWeightedLevel(this.getPlayer(player).getProfile());
        return this.colorizeLevel(level);
    }

    public int getCleanWeightedLevel(Player player) {
        return this.calculateWeightedLevel(this.getPlayer(player).getProfile());
    }

    public int getUnmodifiedLevel(Player player) {
        return this.getPlayer(player).getPowerLevel();
    }
    ...
    private int calculateWeightedLevel(PlayerProfile mcMMOProfile) {
        int swords = this.getLevel(mcMMOProfile, PrimarySkillType.SWORDS);
        int axes = this.getLevel(mcMMOProfile, PrimarySkillType.AXES);
        int unarmed = this.getLevel(mcMMOProfile, PrimarySkillType.UNARMED);
        int archery = this.getLevel(mcMMOProfile, PrimarySkillType.ARCHERY);
        int taming = this.getLevel(mcMMOProfile, PrimarySkillType.TAMING);
        int acrobatics = this.getLevel(mcMMOProfile, PrimarySkillType.ACROBATICS);
    ...
    public int getSkillXP(Player player, PrimarySkillType skill) {
        return this.getPlayer(player).getSkillXpLevel(skill);
    }

    public int getNeededXP(Player player, PrimarySkillType skill) {
        return this.getPlayer(player).getXpToLevel(skill);
    }
