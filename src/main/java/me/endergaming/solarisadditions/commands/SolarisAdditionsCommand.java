...
public class SolarisAdditionsCommand extends MainCommand {
    public SolarisAdditionsCommand(@NotNull SolarisAdditions plugin, String command) {
    ...
    public List<String> onTabComplete(CommandSender sender, Command cmd, String label, String[] args) {
        if (!sender.hasPermission("sa.admin")) return null;
        ...
        if (args.length == 3) {
            if (args[0].equalsIgnoreCase("managers")) {
                if (args[1].matches("reload|register|unregister|reqs")) {
                    return Managers.availableManagers.keySet().stream()
                            .map(String::toLowerCase)
